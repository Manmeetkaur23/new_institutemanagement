...
class School extends Dbconfig {	
	...
	/*****************News and Events action methods****************/
	public function listNewsEvents(){		
		$sqlQuery = "SELECT * FROM ".$this->newsEventsTable." as n ";
		if(!empty($_POST["search"]["value"])){
			$sqlQuery .= ' WHERE (n.id LIKE "%'.$_POST["search"]["value"].'%" ';
			$sqlQuery .= ' OR n.title LIKE "%'.$_POST["search"]["value"].'%" ';					
		}
		if(!empty($_POST["order"])){
			$sqlQuery .= 'ORDER BY '.$_POST['order']['0']['column'].' '.$_POST['order']['0']['dir'].' ';
		} else {
		...
		$result = mysqli_query($this->dbConnect, $sqlQuery);
		$numRows = mysqli_num_rows($result);
		$newsEventsData = array();	
		while( $newsEvents = mysqli_fetch_assoc($result) ) {		
			$newsEventsRows = array();			
			$newsEventsRows[] = $newsEvents['id'];
			$newsEventsRows[] = $newsEvents['title'];
			$newsEventsRows[] = $newsEvents['body'];
			$newsEventsRows[] = $newsEvents['date'];	
			$newsEventsRows[] = $newsEvents['type'];				
			$newsEventsRows[] = '<button type="button" name="update" id="'.$newsEvents["id"].'" class="btn btn-warning btn-xs update">Update</button>';
			$newsEventsRows[] = '<button type="button" name="delete" id="'.$newsEvents["id"].'" class="btn btn-danger btn-xs delete" >Delete</button>';
			$newsEventsData[] = $newsEventsRows;
		}
		$output = array(
			"draw"				=>	intval(isset($_POST["draw"]) ? $_POST["draw"] : 0),
			"recordsTotal"  	=>  $numRows,
	...
	public function addNewsEvents () {
		if($_POST["newseventstitle"]) {
			$insertQuery = "INSERT INTO ".$this->newsEventsTable."(title, body, date, type) 
				VALUES ('".$_POST["newseventstitle"]."', '".$_POST["newseventsbody"]."', '".$_POST["newseventsdate"]."', '".$_POST["newseventstype"]."')";
			$userSaved = mysqli_query($this->dbConnect, $insertQuery);
		}
	}
	public function getNewsEvents(){
		$sqlQuery = "
			SELECT * FROM ".$this->newsEventsTable." 
			WHERE id = '".$_POST["newseventsid"]."'";
		$result = mysqli_query($this->dbConnect, $sqlQuery);	
		$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
		echo json_encode($row);
	}
	public function updateNewsEvents() {
		if($_POST['newseventsid']) {	
			$updateQuery = "UPDATE ".$this->newsEventsTable." 
			SET title = '".$_POST["newseventstitle"]."', 
			body = '".$_POST["newseventsbody"]."',
			date = '".$_POST["newseventsdate"]."',
			type = '".$_POST["newseventstype"]."'
			WHERE id ='".$_POST["newseventsid"]."'";
			$isUpdated = mysqli_query($this->dbConnect, $updateQuery);		
		}	
	}	
	public function deleteNewsEvents(){
		if($_POST["newseventsid"]) {
			$sqlUpdate = "
				DELETE FROM ".$this->newsEventsTable."
				WHERE id = '".$_POST["newseventsid"]."'";		
			mysqli_query($this->dbConnect, $sqlUpdate);		
		}
	}
